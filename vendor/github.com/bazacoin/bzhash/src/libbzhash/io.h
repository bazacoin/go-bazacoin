...
// Reference: https://github.com/bazacoin/wiki/wiki/Bzhash-DAG
#define DAG_MUTABLE_NAME_MAX_SIZE (6 + 10 + 1 + 16 + 1)
/// Possible return values of @see bzhash_io_prepare
enum bzhash_io_rc {
	BZHASH_IO_FAIL = 0,           ///< There has been an IO failure
	BZHASH_IO_MEMO_SIZE_MISMATCH, ///< DAG with revision/hash match, but file size was wrong.
	BZHASH_IO_MEMO_MISMATCH,      ///< The DAG file did not exist or there was revision/hash mismatch
	BZHASH_IO_MEMO_MATCH,         ///< DAG file existed and revision/hash matched. No need to do anything
};
...
/**
 * Logs a critical error in important parts of bzhash. Should mostly help
 * figure out what kind of problem (I/O, memory e.t.c.) causes a NULL
 * bzhash_full_t
 */
#ifdef BZHASH_PRINT_CRITICAL_OUTPUT
#define BZHASH_CRITICAL(...)							\
	do													\
	{													\
		printf("BZHASH CRITICAL ERROR: "__VA_ARGS__);	\
		printf("\n");									\
		fflush(stdout);									\
	} while (0)
#else
...
/**
 * Prepares io for bzhash
 *
 * Create the DAG directory and the DAG file if they don't exist.
 *
 * @param[in] dirname        A null terminated c-string of the path of the bzhash
 ...
 *                           https://github.com/bazacoin/wiki/wiki/Bzhash-DAG
 ...
 *                           already exists
 * @return                   For possible return values @see enum bzhash_io_rc
 */
enum bzhash_io_rc bzhash_io_prepare(
	char const* dirname,
	bzhash_h256_t const seedhash,
	FILE** output_file,
	uint64_t file_size,
	bool force_create
);
 ...
 */
FILE* bzhash_fopen(char const* file_name, char const* mode);
 ...
 *                         error returns NULL
 */
char* bzhash_strncat(char* dest, size_t dest_size, char const* src, size_t count);
 ...
 *                       existed
 */
bool bzhash_mkdir(char const* dirname);
 ...
 */
bool bzhash_file_size(FILE* f, size_t* ret_size);
 ...
 */
char* bzhash_io_create_filename(
	char const* dirname,
	char const* filename,
 ...
 *
 * The spec defining this directory is here: https://github.com/bazacoin/wiki/wiki/Bzhash-DAG
 *
 ...
 */
bool bzhash_get_default_dirname(char* strbuf, size_t buffsize);

static inline bool bzhash_io_mutable_name(
	uint32_t revision,
