...
// Package bzcstats implements the network stats reporting service.
package bzcstats
...
import (
	...
	"github.com/bazacoin/go-bazacoin/common/mclock"
...
// Service implements an Bazacoin netstats reporting daemon that pushes local
// chain statistics up to a monitoring server.
type Service struct {
	stack *node.Node // Temporary workaround, remove when API finalized

	server *p2p.Server        // Peer-to-peer server to retrieve networking infos
	bzc    *bzc.Bazacoin      // Full Bazacoin service if monitoring a full node
	les    *les.LightBazacoin // Light Bazacoin service if monitoring a light node
...
func New(url string, bzcServ *bzc.Bazacoin, lesServ *les.LightBazacoin) (*Service, error) {
	...
	for {
		...
		if len(msg["emit"]) == 2 && command == "history" {
			...
			if !ok {
				...
				s.histCh <- nil
				continue // Bzcstats sometime sends invalid history requests, ignore those
			}
...
func (s *Service) login(conn *websocket.Conn) error {
	...
	if info := infos.Protocols["bzc"]; info != nil {
		network = fmt.Sprintf("%d", info.(*bzc.BzcNodeInfo).Network)
		protocol = fmt.Sprintf("bzc/%d", bzc.ProtocolVersions[0])
	} else {
		network = fmt.Sprintf("%d", infos.Protocols["les"].(*bzc.BzcNodeInfo).Network)
...
func (s *Service) reportLatency(conn *websocket.Conn) error {
	// Send the current time to the bzcstats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to bzcstats", "latency", latency)
...
func (s *Service) reportBlock(conn *websocket.Conn, block *types.Block) error {
	...
	log.Trace("Sending new block to bzcstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) assembleBlockStats(block *types.Block) *blockStats {
	...
	if s.bzc != nil {
		...
		if block == nil {
			block = s.bzc.BlockChain().CurrentBlock()
		}
		header = block.Header()
		td = s.bzc.BlockChain().GetTd(header.Hash(), header.Number.Uint64())
...
func (s *Service) reportHistory(conn *websocket.Conn, list []uint64) error {
	...
	if len(list) > 0 {
		...
		var head int64
		if s.bzc != nil {
			head = s.bzc.BlockChain().CurrentHeader().Number.Int64()
		} else {
	...
	for i, number := range indexes {
		...
		if s.bzc != nil {
			block = s.bzc.BlockChain().GetBlockByNumber(number)
		} else {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to bzcstats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *websocket.Conn) error {
	...
	var pending int
	if s.bzc != nil {
		pending, _ = s.bzc.TxPool().Stats()
	} else {
		pending = s.les.TxPool().Stats()
	}
	// Assemble the transaction stats and send it to the server
	log.Trace("Sending pending transactions to bzcstats", "count", pending)
...
func (s *Service) reportStats(conn *websocket.Conn) error {
	...
	if s.bzc != nil {
		mining = s.bzc.Miner().Mining()
		hashrate = int(s.bzc.Miner().HashRate())

		sync := s.bzc.Downloader().Progress()
		syncing = s.bzc.BlockChain().CurrentHeader().Number.Uint64() >= sync.HighestBlock

		price, _ := s.bzc.ApiBackend.SuggestPrice(context.Background())
	...
	log.Trace("Sending node details to bzcstats")
