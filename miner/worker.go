...
func newWorker(config *params.ChainConfig, engine consensus.Engine, coinbase common.Address, bzc Backend, mux *event.TypeMux) *worker {
	worker := &worker{
		config:         config,
		engine:         engine,
		bzc:            bzc,
		...
		chain:          bzc.BlockChain(),
		proc:           bzc.BlockChain().Validator(),
		possibleUncles: make(map[common.Hash]*types.Block),
		coinbase:       coinbase,
		txQueue:        make(map[common.Hash]*types.Transaction),
		agents:         make(map[Agent]struct{}),
		unconfirmed:    newUnconfirmedBlocks(bzc.BlockChain(), 5),
...
func (self *worker) setBazacoinbase(addr common.Address) {
	self.mu.Lock()
...
func (self *worker) makeCurrent(parent *types.Block, header *types.Header) error {
	...
	wallets := self.bzc.AccountManager().Wallets()
...
func (self *worker) commitNewWork() {
	...
	pending, err := self.bzc.TxPool().Pending()
	if err != nil {
	...
	self.bzc.TxPool().RemoveBatch(work.failedTxs)
