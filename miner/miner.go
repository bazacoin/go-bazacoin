...
// Package miner implements Bazacoin block creation and mining.
package miner
...
func New(bzc Backend, config *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine) *Miner {
	miner := &Miner{
		bzc:      bzc,
		mux:      mux,
		engine:   engine,
		worker:   newWorker(config, engine, common.Address{}, bzc, mux),
		canStart: 1,
	}
	miner.Register(NewCpuAgent(bzc.BlockChain(), engine))
	go miner.update()
...
func (self *Miner) Start(coinbase common.Address) {
	atomic.StoreInt32(&self.shouldStart, 1)
	self.worker.setBazacoinbase(coinbase)
...
func (self *Miner) SetBazacoinbase(addr common.Address) {
	self.coinbase = addr
	self.worker.setBazacoinbase(addr)
}
