...
while [ 1 ]
do	
	TEST="$(docker run --rm --entrypoint=\"/cpp-bazacoin/build/test/createRandomStateTest\" bazacoin/cppjit-testrunner)"
	# echo "$TEST"
	
	# test pybazacoin
	OUTPUT_PYTHON="$(docker run --rm bazacoin/python-testrunner --notrace <<< "$TEST")"
	RESULT_PYTHON=$?

	# test go
	OUTPUT_GO="$(docker run --rm bazacoin/go-testrunner "$TEST")"
	RESULT_GO=$?
	
	# test cpp-jit
	OUTPUT_CPPJIT="$(docker run --rm bazacoin/cppjit-testrunner "$TEST")"
	RESULT_CPPJIT=$?
