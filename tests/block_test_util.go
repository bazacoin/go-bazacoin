...
func runBlockTest(homesteadBlock, daoForkBlock, gasPriceFork *big.Int, test *BlockTest) error {
	...
	chain, err := core.NewBlockChain(db, config, bzhash.NewShared(), evmux, vm.Config{})
	if err != nil {
		return err
	}
	defer chain.Stop()
...
func (t *BlockTest) InsertPreState(db bzcdb.Database) (*state.StateDB, error) {
...
/* See https://github.com/bazacoin/tests/wiki/Blockchain-Tests-II
