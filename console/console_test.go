...
func newTester(t *testing.T, confOverride func(*bzc.Config)) *tester {
	...
	// Create a networkless protocol stack and start an Bazacoin service within
	...
	bzcConf := &bzc.Config{
		...
		Bazacoinbase: common.HexToAddress(testAddress),
		PowTest:   true,
	}
	if confOverride != nil {
		confOverride(bzcConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return bzc.New(ctx, bzcConf) }); err != nil {
		t.Fatalf("failed to register Bazacoin protocol: %v", err)
	}
