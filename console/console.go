...
import (
	...
	"github.com/bazacoin/go-bazacoin/internal/jsre"
	"github.com/bazacoin/go-bazacoin/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client  // RPC client to execute Bazacoin requests through
...
func (c *Console) init(preload []string) error {
	...
	c.jsre.Set("jbzc", struct{}{})

	jbzcObj, _ := c.jsre.Get("jbzc")
	jbzcObj.Object().Set("send", bridge.Send)
	jbzcObj.Object().Set("sendAsync", bridge.Send)
	...
	if _, err := c.jsre.Run("var web3 = new Web3(jbzc);"); err != nil {
	...
	flatten := "var bzc = web3.bzc; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = bzc.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
	...
	if c.prompter != nil {
		...
		// Assign these method in the Console the original web3 callbacks. These will be called by the jbzc.*
		// methods after they got the password from the user and send the original web3 request to the backend.
		if obj := personal.Object(); obj != nil { // make sure the personal api is enabled over the interface
			if _, err = c.jsre.Run(`jbzc.unlockAccount = personal.unlockAccount;`); err != nil {
				return fmt.Errorf("personal.unlockAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jbzc.newAccount = personal.newAccount;`); err != nil {
				return fmt.Errorf("personal.newAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jbzc.sign = personal.sign;`); err != nil {
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines bzc.getBalance(bzc.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
func (c *Console) Welcome() {
	...
	c.jsre.Run(`
		...
		console.log("coinbase: " + bzc.coinbase);
		console.log("at block: " + bzc.blockNumber + " (" + new Date(1000 * bzc.getBlock(bzc.blockNumber).timestamp) + ")");
