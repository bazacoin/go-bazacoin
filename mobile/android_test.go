...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.bazacoin", new NodeConfig());
			node.start();
			...
			BazacoinClient ec = node.getBazacoinClient();
	...
	// signatures too. Regression test for go-bazacoin issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/bazacoin/gethtest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.bazacoin", "github.com/bazacoin/go-bazacoin/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "bazacoin", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
