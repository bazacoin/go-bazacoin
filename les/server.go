...
import (
	...
	"github.com/bazacoin/go-bazacoin/les/flowcontrol"
...
func NewLesServer(bzc *bzc.Bazacoin, config *bzc.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(bzc.BlockChain().Config(), false, config.NetworkId, bzc.EventMux(), bzc.Engine(), newPeerSet(), bzc.BlockChain(), bzc.TxPool(), bzc.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	pm.blockLoop()
	...
	srv := &LesServer{
		protocolManager: pm,
		quitSync:        quitSync,
		lesTopic:        lesTopic(bzc.BlockChain().Genesis().Hash()),
	}
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(bzc.ChainDb())
	return srv, nil
}
...
func newCostStats(db bzcdb.Database) *requestCostStats {
...
func getChtRoot(db bzcdb.Database, num uint64) common.Hash {
...
func storeChtRoot(db bzcdb.Database, num uint64, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], num)
	db.Put(append(chtPrefix, encNumber[:]...), root[:])
}

func makeCht(db bzcdb.Database) bool {
