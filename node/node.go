...
func (n *Node) OpenDatabase(name string, cache, handles int) (bzcdb.Database, error) {
	if n.config.DataDir == "" {
		return bzcdb.NewMemDatabase()
	}
	return bzcdb.NewLDBDatabase(n.config.resolvePath(name), cache, handles)
}
