...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (bzcdb.Database, error) {
	if ctx.config.DataDir == "" {
		return bzcdb.NewMemDatabase()
	}
	db, err := bzcdb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
