...
type SimulatedBackend struct {
	database   bzcdb.Database   // In memory database to store our testing data
	blockchain *core.BlockChain // Bazacoin blockchain to handle the consensus
...
func NewSimulatedBackend(alloc core.GenesisAlloc) *SimulatedBackend {
	database, _ := bzcdb.NewMemDatabase()
	genesis := core.Genesis{Config: params.AllProtocolChanges, Alloc: alloc}
	genesis.MustCommit(database)
	blockchain, _ := core.NewBlockChain(database, genesis.Config, bzhash.NewFaker(), new(event.TypeMux), vm.Config{})
...
func (b *SimulatedBackend) CallContract(ctx context.Context, call bazacoin.CallMsg, blockNumber *big.Int) ([]byte, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) PendingCallContract(ctx context.Context, call bazacoin.CallMsg) ([]byte, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) EstimateGas(ctx context.Context, call bazacoin.CallMsg) (*big.Int, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) callContract(ctx context.Context, call bazacoin.CallMsg, block *types.Block, statedb *state.StateDB) ([]byte, *big.Int, error) {
