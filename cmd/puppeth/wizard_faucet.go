...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many Bazacoins to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.bzcstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.bzcstats = w.readString() + ":" + w.conf.bzcstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.bzcstats)
		infos.node.bzcstats = w.readDefaultString(infos.node.bzcstats) + ":" + w.conf.bzcstats
	}
