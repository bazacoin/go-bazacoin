...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"bzcstats", "explorer", "wallet", "faucet"} {
		...
		for _, server := range available[service] {
			...
			switch service {
			case "bzcstats":
				if infos, err := checkBzcstats(client, w.network); err == nil {
					port = infos.port
				}
			case "faucet":
	...
	// If we have bzcstats running, ask whether to make the secret public or not
	var bzcstats bool
	if w.conf.bzcstats != "" {
		fmt.Println()
		fmt.Println("Include bzcstats secret on dashboard (y/n)? (default = yes)")
		bzcstats = w.readDefaultString("y") == "y"
	}
	// Try to deploy the dashboard container on the host
	if out, err := deployDashboard(client, w.network, infos.port, infos.host, listing, &w.conf, bzcstats); err != nil {
