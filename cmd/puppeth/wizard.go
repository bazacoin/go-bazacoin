...
type config struct {
	...
	bzcstats  string        // Bzcstats settings to cache for node deploys
...
type wizard struct {
	...
	services map[string][]string   // Bazacoin services known to be running on servers
...
// converts it to an Bazacoin address. If an empty line is entered, the default
// value is returned.
