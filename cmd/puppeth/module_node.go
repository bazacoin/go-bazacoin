...
// nodeDockerfile is the Dockerfile required to run an Bazacoin node.
var nodeDockerfile = `
FROM bazacoin/client-go:alpine-develop
...
RUN \
  echo '/geth init /genesis.json' > geth.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.bazacoin/keystore/ && cp /signer.json /root/.bazacoin/keystore/' >> geth.sh && \{{end}}
	echo $'/geth --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --maxpeers {{.Peers}} {{.LightFlag}} --bzcstats \'{{.Bzcstats}}\' {{if .BootV4}}--bootnodesv4 {{.BootV4}}{{end}} {{if .BootV5}}--bootnodesv5 {{.BootV5}}{{end}} {{if .Bazacoinbase}}--bazacoinbase {{.Bazacoinbase}} --mine{{end}}{{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --targetgaslimit {{.GasTarget}} --gasprice {{.GasPrice}}' >> geth.sh

ENTRYPOINT ["/bin/sh", "geth.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an Bazacoin node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    environment:
      ...
      - STATS_NAME={{.Bzcstats}}
      - MINER_NAME={{.Bazacoinbase}}
      - GAS_TARGET={{.GasTarget}}
      - GAS_PRICE={{.GasPrice}}
    restart: always
`

// deployNode deploys a new Bazacoin node container to a remote machine via SSH,
// docker and docker-compose. If an instance with the specified network name
// already exists there, it will be overwritten!
func deployNode(client *sshClient, network string, bootv4, bootv5 []string, config *nodeInfos) ([]byte, error) {
	kind := "sealnode"
	if config.keyJSON == "" && config.bazacoinbase == "" {
		kind = "bootnode"
	...
	template.Must(template.New("").Parse(nodeComposefile)).Execute(composefile, map[string]interface{}{
		...
		"Bzcstats":   config.bzcstats[:strings.Index(config.bzcstats, ":")],
		"Bazacoinbase":  config.bazacoinbase,
...
func (info *nodeInfos) String() string {
	...
	return fmt.Sprintf("port=%d%s, datadir=%s, peers=%d, lights=%d, bzcstats=%s, gastarget=%0.3f MGas, gasprice=%0.3f GWei",
		info.portFull, discv5, info.datadir, info.peersTotal, info.peersLight, info.bzcstats, info.gasTarget, info.gasPrice)
}
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.bazacoin"],
		portFull:   infos.portmap[infos.envvars["FULL_PORT"]],
		portLight:  infos.portmap[infos.envvars["LIGHT_PORT"]],
		peersTotal: totalPeers,
		peersLight: lightPeers,
		bzcstats:   infos.envvars["STATS_NAME"],
		bazacoinbase:  infos.envvars["MINER_NAME"],
		keyJSON:    keyJSON,
		keyPass:    keyPass,
		gasTarget:  gasTarget,
		gasPrice:   gasPrice,
