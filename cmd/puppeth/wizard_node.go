...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.bzcstats == "" {
		log.Error("No bzcstats server configured")
		return
	}
	...
	if infos.bzcstats == "" {
		...
		infos.bzcstats = w.readString() + ":" + w.conf.bzcstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.bzcstats)
		infos.bzcstats = w.readDefaultString(infos.bzcstats) + ":" + w.conf.bzcstats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.genesis.Config.Bzhash != nil {
			// Bzhash based miners only need an bazacoinbase to mine against
			fmt.Println()
			if infos.bazacoinbase == "" {
				fmt.Printf("What address should the miner user?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.bazacoinbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner user? (default = %s)\n", infos.bazacoinbase)
				infos.bazacoinbase = w.readDefaultAddress(common.HexToAddress(infos.bazacoinbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootFull, w.conf.bootLight, infos); err != nil {
		log.Error("Failed to deploy Bazacoin node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
