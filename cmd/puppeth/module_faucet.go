...
RUN \
  apk add --update git go make gcc musl-dev ca-certificates linux-headers                             && \
	mkdir -p $GOPATH/src/github.com/bazacoin                                                            && \
	(cd $GOPATH/src/github.com/bazacoin && git clone --depth=1 https://github.com/bazacoin/go-bazacoin) && \
  go build -v github.com/bazacoin/go-bazacoin/cmd/faucet                                              && \
...
CMD [ \
	"/faucet", "--genesis", "/genesis.json", "--network", "{{.NetworkID}}", "--bootnodes", "{{.Bootnodes}}", "--bzcstats", "{{.Bzcstats}}", "--bzcport", "{{.BzcPort}}", \
...
func deployFaucet(client *sshClient, network string, bootnodes []string, config *faucetInfos) ([]byte, error) {
	...
	template.Must(template.New("").Parse(faucetDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"Bzcstats":      config.node.bzcstats,
		"BzcPort":       config.node.portFull,
	...
	template.Must(template.New("").Parse(faucetComposefile)).Execute(composefile, map[string]interface{}{
		...
		"ApiPort":       config.port,
		"BzcPort":       config.node.portFull,
		"BzcName":       config.node.bzcstats[:strings.Index(config.node.bzcstats, ":")],
		"GitHubUser":    config.githubUser,
		"GitHubToken":   config.githubToken,
		"CaptchaToken":  config.captchaToken,
		"CaptchaSecret": config.captchaSecret,
...
func (info *faucetInfos) String() string {
	return fmt.Sprintf("host=%s, api=%d, bzc=%d, amount=%d, minutes=%d, tiers=%d, github=%s, captcha=%v, bzcstats=%s", info.host, info.port, info.node.portFull, info.amount, info.minutes, info.tiers, info.githubUser, info.captchaToken != "", info.node.bzcstats)
}
...
func checkFaucet(client *sshClient, network string) (*faucetInfos, error) {
	...
	return &faucetInfos{
		node: &nodeInfos{
			...
			portFull: infos.portmap[infos.envvars["BZC_PORT"]+"/tcp"],
			bzcstats: infos.envvars["BZC_NAME"],
			keyJSON:  keyJSON,
			keyPass:  keyPass,
		},
		host:          host,
