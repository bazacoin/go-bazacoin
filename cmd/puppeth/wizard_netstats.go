...
func (w *wizard) networkStats(tips bool) {
	...
	for server, pubkey := range w.conf.Servers {
		...
		logger.Debug("Checking for bzcstats availability")
		if infos, err := checkBzcstats(client, w.network); err != nil {
			if err != ErrServiceUnknown {
				services["bzcstats"] = err.Error()
			}
		} else {
			services["bzcstats"] = infos.String()
			protips.bzcstats = infos.config
		}
	...
	if protips.bzcstats != "" {
		w.conf.bzcstats = protips.bzcstats
	}
...
func (p *protips) print(network string) {
	...
	// If an bzcstats server is available, add the bzcstats flag
	statsflag := ""
	if p.bzcstats != "" {
		if strings.Contains(p.bzcstats, " ") {
			statsflag = fmt.Sprintf(` --bzcstats="yournode:%s"`, p.bzcstats)
		} else {
			statsflag = fmt.Sprintf(` --bzcstats=yournode:%s`, p.bzcstats)
		}
	}
