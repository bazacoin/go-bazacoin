...
import (
	...
	"github.com/bazacoin/go-bazacoin/swarm"
	bzzapi "github.com/bazacoin/go-bazacoin/swarm/api"
...
var (
	...
	BzcAPIFlag = cli.StringFlag{
		Name:  "bzcapi",
		Usage: "URL of the Bazacoin API provider",
...
var app = utils.NewApp(gitCommit, "Bazacoin Swarm")

// This init function creates the cli.App.
func init() {
	app.Action = bzzd
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2013-2016 The go-bazacoin Authors"
...
func registerBzzService(ctx *cli.Context, stack *node.Node) {
	...
	bzcapi := ctx.GlobalString(BzcAPIFlag.Name)
	cors := ctx.GlobalString(CorsStringFlag.Name)

	boot := func(ctx *node.ServiceContext) (node.Service, error) {
		var client *bzcclient.Client
		if len(bzcapi) > 0 {
			client, err = bzcclient.Dial(bzcapi)
			if err != nil {
