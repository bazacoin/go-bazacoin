...
// faucet is a Bazacoin faucet backed by a light client.
package main
...
var (
	...
	bzcPortFlag = flag.Int("bzcport", 30303, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Bazacoin protocol")
	statsFlag   = flag.String("bzcstats", "", "Bzcstats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Bazacoins to pay out per user request")
	...
	logFlag = flag.Int("loglevel", 3, "Log level to use for Bazacoin and the faucet")
)

var (
	bazacoin = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Bazacoins", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
	...
	faucet, err := newFaucet(genesis, *bzcPortFlag, enodes, *netFlag, *statsFlag, ks, website.Bytes())
	if err != nil {
...
type request struct {
	...
	Account  common.Address     `json:"account"`  // Bazacoin address being funded
	Time     time.Time          `json:"time"`     // Timestamp when te request was accepted
	Tx       *types.Transaction `json:"tx"`       // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Bazacoin light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Bazacoin protocol stack
	client *bzcclient.Client   // Client connection to the Bazacoin chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Bazacoin light client protocol
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		cfg := bzc.DefaultConfig
	...
	// Assemble the bzcstats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightBazacoin
			ctx.Service(&serv)
			return bzcstats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := bzcclient.NewClient(api)
...
// close terminates the Bazacoin connection and tears down the faucet.
...
// apiHandler handles requests for Bazacoin grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	websocket.JSON.Send(conn, map[string]interface{}{
		"funds":    balance.Div(balance, bazacoin),
		"funded":   nonce,
	...
	for {
		...
		// Iterate over all the files and look for Bazacoin addresses
		...
		if address == (common.Address{}) {
			websocket.JSON.Send(conn, map[string]string{"error": "No Bazacoin address found to fund"})
			continue
		}
		...
		if timeout = f.timeouts[gist.Owner.Login]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), bazacoin)
...
func (f *faucet) loop() {
	...
	for {
		...
		case head := <-heads:
			...
			balance, _ := f.client.BalanceAt(context.Background(), f.account.Address, nil)
			balance = new(big.Int).Div(balance, bazacoin)
