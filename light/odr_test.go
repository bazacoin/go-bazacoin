...
type testOdr struct {
	OdrBackend
	sdb, ldb bzcdb.Database
	disable  bool
}

func (odr *testOdr) Database() bzcdb.Database {
	return odr.ldb
}
...
type odrTestFn func(ctx context.Context, db bzcdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) []byte

func TestOdrGetBlockLes1(t *testing.T) { testChainOdr(t, 1, 1, odrGetBlock) }

func odrGetBlock(ctx context.Context, db bzcdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) []byte {
...
func odrGetReceipts(ctx context.Context, db bzcdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) []byte {
...
func odrAccounts(ctx context.Context, db bzcdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) []byte {
...
func odrContractCall(ctx context.Context, db bzcdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) []byte {
...
func testChainGen(i int, block *core.BlockGen) {
	...
	switch i {
	case 0:
		// In block 1, the test bank sends account #1 some bazacoin.
		...
		block.AddTx(tx)
	case 1:
		// In block 2, the test bank sends some more bazacoin to account #1.
...
func testChainOdr(t *testing.T, protocol int, expFail uint64, fn odrTestFn) {
	var (
		...
		sdb, _  = bzcdb.NewMemDatabase()
		ldb, _  = bzcdb.NewMemDatabase()
		gspec   = core.Genesis{Alloc: core.GenesisAlloc{testBankAddress: {Balance: testBankFunds}}}
		genesis = gspec.MustCommit(sdb)
	)
	gspec.MustCommit(ldb)
	// Assemble the test environment
	blockchain, _ := core.NewBlockChain(sdb, params.TestChainConfig, bzhash.NewFullFaker(), evmux, vm.Config{})
	...
	lightchain, _ := NewLightChain(odr, params.TestChainConfig, bzhash.NewFullFaker(), evmux)
