...
// header only chain.
func newCanonical(n int) (bzcdb.Database, *LightChain, error) {
	db, _ := bzcdb.NewMemDatabase()
	gspec := core.Genesis{Config: params.TestChainConfig}
	genesis := gspec.MustCommit(db)
	blockchain, _ := NewLightChain(&dummyOdr{db: db}, gspec.Config, bzhash.NewFaker(), new(event.TypeMux))
...
func newTestLightChain() *LightChain {
	...
	gspec.MustCommit(db)
	lc, err := NewLightChain(&dummyOdr{db: db}, gspec.Config, bzhash.NewFullFaker(), new(event.TypeMux))
	if err != nil {
		panic(err)
	}
	return lc
...
func (odr *dummyOdr) Database() bzcdb.Database {
	return odr.db
}
...
func TestReorgBadHeaderHashes(t *testing.T) {
	...
	ncm, err := NewLightChain(&dummyOdr{db: bc.chainDb}, params.TestChainConfig, bzhash.NewFaker(), new(event.TypeMux))
	if err != nil {
