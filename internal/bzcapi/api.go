...
// PublicBazacoinAPI provides an API to access Bazacoin related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBazacoinAPI struct {
	b Backend
}

// NewPublicBazacoinAPI creates a new Bazacoin protocol API.
func NewPublicBazacoinAPI(b Backend) *PublicBazacoinAPI {
	return &PublicBazacoinAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicBazacoinAPI) GasPrice(ctx context.Context) (*big.Int, error) {
	return s.b.SuggestPrice(ctx)
}

// ProtocolVersion returns the current Bazacoin protocol version this node supports
func (s *PublicBazacoinAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicBazacoinAPI) Syncing() (interface{}, error) {
...
//   keccak256("\x19Bazacoin Signed Message:\n"${message length}${message}).
//
// This gives context to the signed message and prevents signing of transactions.
func signHash(data []byte) []byte {
	msg := fmt.Sprintf("\x19Bazacoin Signed Message:\n%d%s", len(data), data)
	return crypto.Keccak256([]byte(msg))
}

// Sign calculates an Bazacoin ECDSA signature for:
// keccack256("\x19Bazacoin Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/bazacoin/go-bazacoin/wiki/Management-APIs#personal_sign
...
// Note, this function is compatible with bzc_sign and personal_sign. As such it recovers
// the address of:
// hash = keccak256("\x19Bazacoin Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be be 27 or 28 for legacy reasons.
//
// https://github.com/bazacoin/go-bazacoin/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != 65 {
		return common.Address{}, fmt.Errorf("signature must be 65 bytes long")
	}
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid Bazacoin signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the Bazacoin blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Bazacoin blockchain API.
...
func getTransaction(chainDb bzcdb.Database, b Backend, txHash common.Hash) (*types.Transaction, bool, error) {
...
func getTransactionBlockData(chainDb bzcdb.Database, txHash common.Hash) (common.Hash, uint64, uint64, error) {
	var txBlock struct {
...
//
// https://github.com/bazacoin/wiki/wiki/JSON-RPC#bzc_sign
...
// PublicDebugAPI is the collection of Bazacoin APIs exposed over the public
// debugging endpoint.
...
func (api *PublicDebugAPI) SeedHash(ctx context.Context, number uint64) (string, error) {
	...
	return fmt.Sprintf("0x%x", bzhash.SeedHash(number)), nil
}

// PrivateDebugAPI is the collection of Bazacoin APIs exposed over the private
// debugging endpoint.
...
// Version returns the current bazacoin protocol version.
