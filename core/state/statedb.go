...
// Package state provides a caching layer atop the Bazacoin state trie.
package state
...
// StateDBs within the bazacoin protocol are used to store anything
...
func New(root common.Hash, db bzcdb.Database) (*StateDB, error) {
...
//
// Carrying over the balance ensures that Bazacoin doesn't disappear.
...
func (s *StateDB) CommitBatch(deleteEmptyObjects bool) (root common.Hash, batch bzcdb.Batch) {
	batch = s.db.NewBatch()
