...
// genTxRing returns a block generator that sends bazacoin in a ring
...
func benchInsertChain(b *testing.B, disk bool, gen func(int, *BlockGen)) {
	...
	if !disk {
		...
		db, err = bzcdb.NewLDBDatabase(dir, 128, 128)
		if err != nil {
	...
	chainman, _ := NewBlockChain(db, gspec.Config, bzhash.NewFaker(), evmux, vm.Config{})
	defer chainman.Stop()
	b.ReportAllocs()
	b.ResetTimer()
...
// into a database.
func makeChainForBench(db bzcdb.Database, full bool, count uint64) {
...
func benchWriteChain(b *testing.B, full bool, count uint64) {
	for i := 0; i < b.N; i++ {
		...
		db, err := bzcdb.NewLDBDatabase(dir, 128, 1024)
		if err != nil {
...
func benchReadChain(b *testing.B, full bool, count uint64) {
	...
	for i := 0; i < b.N; i++ {
		...
		chain, err := NewBlockChain(db, params.TestChainConfig, bzhash.NewFaker(), new(event.TypeMux), vm.Config{})
		if err != nil {
