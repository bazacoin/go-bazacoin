...
import (
	...
	"github.com/bazacoin/go-bazacoin/trie"
)

const defaultTraceTimeout = 5 * time.Second

// PublicBazacoinAPI provides an API to access Bazacoin full node-related
// information.
type PublicBazacoinAPI struct {
	e *Bazacoin
}

// NewPublicBazacoinAPI creates a new Bazacoin protocol API for full nodes.
func NewPublicBazacoinAPI(e *Bazacoin) *PublicBazacoinAPI {
	return &PublicBazacoinAPI{e}
}

// Bazacoinbase is the address that mining rewards will be send to
func (api *PublicBazacoinAPI) Bazacoinbase() (common.Address, error) {
	return api.e.Bazacoinbase()
}

// Coinbase is the address that mining rewards will be send to (alias for Bazacoinbase)
func (api *PublicBazacoinAPI) Coinbase() (common.Address, error) {
	return api.Bazacoinbase()
}

// Hashrate returns the POW hashrate
func (api *PublicBazacoinAPI) Hashrate() hexutil.Uint64 {
...
func NewPublicMinerAPI(e *Bazacoin) *PublicMinerAPI {
...
func NewPrivateMinerAPI(e *Bazacoin) *PrivateMinerAPI {
...
// SetBazacoinbase sets the bazacoinbase of the miner
func (api *PrivateMinerAPI) SetBazacoinbase(bazacoinbase common.Address) bool {
	api.e.SetBazacoinbase(bazacoinbase)
	return true
}
...
// PrivateAdminAPI is the collection of Bazacoin full node-related APIs
...
// admin methods of the Bazacoin service.
func NewPrivateAdminAPI(bzc *Bazacoin) *PrivateAdminAPI {
	return &PrivateAdminAPI{bzc: bzc}
}
...
func (api *PrivateAdminAPI) ExportChain(file string) (bool, error) {
	...
	if err := api.bzc.BlockChain().Export(writer); err != nil {
		return false, err
	}
	return true, nil
}
...
func (api *PrivateAdminAPI) ImportChain(file string) (bool, error) {
	...
	for batch := 0; ; batch++ {
		...
		if hasAllBlocks(api.bzc.BlockChain(), blocks) {
			blocks = blocks[:0]
			continue
		}
		// Import the batch and reset the buffer
		if _, err := api.bzc.BlockChain().InsertChain(blocks); err != nil {
...
// PublicDebugAPI is the collection of Bazacoin full node APIs exposed
...
// related public debug methods of the Bazacoin service.
func NewPublicDebugAPI(bzc *Bazacoin) *PublicDebugAPI {
	return &PublicDebugAPI{bzc: bzc}
}
...
func (api *PublicDebugAPI) DumpBlock(blockNr rpc.BlockNumber) (state.Dump, error) {
	if blockNr == rpc.PendingBlockNumber {
		...
		_, stateDb := api.bzc.miner.Pending()
		return stateDb.RawDump(), nil
	}
	var block *types.Block
	if blockNr == rpc.LatestBlockNumber {
		block = api.bzc.blockchain.CurrentBlock()
	} else {
		block = api.bzc.blockchain.GetBlockByNumber(uint64(blockNr))
	}
	if block == nil {
		return state.Dump{}, fmt.Errorf("block #%d not found", blockNr)
	}
	stateDb, err := api.bzc.BlockChain().StateAt(block.Root())
	if err != nil {
...
// PrivateDebugAPI is the collection of Bazacoin full node APIs exposed over
...
// private debug methods of the Bazacoin service.
func NewPrivateDebugAPI(config *params.ChainConfig, bzc *Bazacoin) *PrivateDebugAPI {
	return &PrivateDebugAPI{config: config, bzc: bzc}
}
...
type BlockTraceResult struct {
	...
	StructLogs []bzcapi.StructLogRes `json:"structLogs"`
...
func (api *PrivateDebugAPI) TraceBlock(blockRlp []byte, config *vm.LogConfig) BlockTraceResult {
	...
	return BlockTraceResult{
		Validated:  validated,
		StructLogs: bzcapi.FormatLogs(logs),
...
func (api *PrivateDebugAPI) TraceBlockByNumber(blockNr rpc.BlockNumber, config *vm.LogConfig) BlockTraceResult {
	...
	case rpc.PendingBlockNumber:
		...
		block = api.bzc.miner.PendingBlock()
...
func (api *PrivateDebugAPI) TraceBlockByHash(hash common.Hash, config *vm.LogConfig) BlockTraceResult {
	// Fetch the block that we aim to reprocess
	block := api.bzc.BlockChain().GetBlockByHash(hash)
	if block == nil {
...
func (api *PrivateDebugAPI) traceBlock(block *types.Block, logConfig *vm.LogConfig) (bool, []vm.StructLog, error) {
	...
	if err := api.bzc.engine.VerifyHeader(blockchain, block.Header(), true); err != nil {
...
func (api *PrivateDebugAPI) TraceTransaction(ctx context.Context, txHash common.Hash, config *TraceArgs) (interface{}, error) {
	...
	if config != nil && config.Tracer != nil {
		...
		var err error
		if tracer, err = bzcapi.NewJavascriptTracer(*config.Tracer); err != nil {
			return nil, err
		}
		...
		go func() {
			<-deadlineCtx.Done()
			tracer.(*bzcapi.JavascriptTracer).Stop(&timeoutError{})
		}()
		defer cancel()
	} else if config == nil {
	...
	tx, blockHash, _, txIndex := core.GetTransaction(api.bzc.ChainDb(), txHash)
	if tx == nil {
	...
	case *vm.StructLogger:
		return &bzcapi.ExecutionResult{
			Gas:         gas,
			ReturnValue: fmt.Sprintf("%x", ret),
			StructLogs:  bzcapi.FormatLogs(tracer.StructLogs()),
		}, nil
	case *bzcapi.JavascriptTracer:
...
func (api *PrivateDebugAPI) computeTxEnv(blockHash common.Hash, txIndex int) (core.Message, vm.Context, *state.StateDB, error) {
	...
	block := api.bzc.BlockChain().GetBlockByHash(blockHash)
	if block == nil {
		return nil, vm.Context{}, nil, fmt.Errorf("block %x not found", blockHash)
	}
	parent := api.bzc.BlockChain().GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
		return nil, vm.Context{}, nil, fmt.Errorf("block parent %x not found", block.ParentHash())
	}
	statedb, err := api.bzc.BlockChain().StateAt(parent.Root())
	if err != nil {
	...
	for idx, tx := range txs {
		...
		context := core.NewEVMContext(msg, block.Header(), api.bzc.BlockChain(), nil)
		if idx == txIndex {
...
func (api *PrivateDebugAPI) Preimage(ctx context.Context, hash common.Hash) (hexutil.Bytes, error) {
	db := core.PreimageTable(api.bzc.ChainDb())
...
func (api *PrivateDebugAPI) GetBadBlocks(ctx context.Context) ([]core.BadBlockArgs, error) {
	return api.bzc.BlockChain().BadBlocks()
}
