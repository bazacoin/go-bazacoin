...
// Block header and body fetchers belonging to bzc/62 and above
...
type peer struct {
	...
	getRelHeaders  relativeHeaderFetcherFn // [bzc/62] Method to retrieve a batch of headers from an origin hash
	getAbsHeaders  absoluteHeaderFetcherFn // [bzc/62] Method to retrieve a batch of headers from an absolute position
	getBlockBodies blockBodyFetcherFn      // [bzc/62] Method to retrieve a batch of block bodies

	getReceipts receiptFetcherFn // [bzc/63] Method to retrieve a batch of block transaction receipts
	getNodeData stateFetcherFn   // [bzc/63] Method to retrieve a batch of state trie data

	version int        // Bzc protocol version number to switch strategies
...
func (p *peer) FetchHeaders(from uint64, count int) error {
	...
	if p.version < 62 {
		panic(fmt.Sprintf("header fetch [bzc/62+] requested on bzc/%d", p.version))
	}
...
func (p *peer) FetchBodies(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 62 {
		panic(fmt.Sprintf("body fetch [bzc/62+] requested on bzc/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
func (p *peer) FetchReceipts(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 63 {
		panic(fmt.Sprintf("body fetch [bzc/63+] requested on bzc/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
func (p *peer) FetchNodeData(hashes []common.Hash) error {
	// Sanity check the protocol version
	if p.version < 63 {
		panic(fmt.Sprintf("node data fetch [bzc/63+] requested on bzc/%d", p.version))
	}
	// Short circuit if the peer is already fetching
