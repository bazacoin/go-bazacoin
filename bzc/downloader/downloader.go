...
var (
	...
	maxQueuedHeaders  = 32 * 1024 // [bzc/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	// Channels
	headerCh      chan dataPack        // [bzc/62] Channel receiving inbound block headers
	bodyCh        chan dataPack        // [bzc/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack        // [bzc/63] Channel receiving inbound receipts
	bodyWakeCh    chan bool            // [bzc/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool            // [bzc/63] Channel to signal the receipt fetcher of new tasks
	headerProcCh  chan []*types.Header // [bzc/62] Channel to feed the header processor new tasks

	// for stateFetcher
	stateSyncStart chan *stateSync
	trackStateReq  chan *stateReq
	stateCh        chan dataPack // [bzc/63] Channel receiving inbound node state data
...
func New(mode SyncMode, stateDb bzcdb.Database, mux *event.TypeMux, hasHeader headerCheckFn, hasBlockAndState blockAndStateCheckFn,
...
// these are zero.
func (d *Downloader) Progress() bazacoin.SyncProgress {
...
func (d *Downloader) syncWithPeer(p *peer, hash common.Hash, td *big.Int) (err error) {
	...
	log.Debug("Synchronising with the network", "peer", p.id, "bzc", p.version, "head", hash, "td", td, "mode", d.mode)
