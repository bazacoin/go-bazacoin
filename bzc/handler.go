...
import (
	...
	"github.com/bazacoin/go-bazacoin/bzc/fetcher"
...
// NewProtocolManager returns a new bazacoin sub protocol manager. The Bazacoin sub protocol manages peers capable
// with the bazacoin network.
func NewProtocolManager(config *params.ChainConfig, mode downloader.SyncMode, networkId uint64, maxPeers int, mux *event.TypeMux, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb bzcdb.Database) (*ProtocolManager, error) {
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Bazacoin peer", "peer", id)

	// Unregister the peer from the downloader and Bazacoin peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping Bazacoin protocol")
	...
	log.Info("Bazacoin protocol stopped")
}
...
// handle is the callback invoked to manage the life cycle of an bzc peer. When
// this function terminates, the peer is disconnected.
func (pm *ProtocolManager) handle(p *peer) error {
	if pm.peers.Len() >= pm.maxPeers {
		return p2p.DiscTooManyPeers
	}
	p.Log().Debug("Bazacoin peer connected", "name", p.Name())

	// Execute the Bazacoin handshake
	td, head, genesis := pm.blockchain.Status()
	if err := p.Handshake(pm.networkId, td, head, genesis); err != nil {
		p.Log().Debug("Bazacoin handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Bazacoin peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Bazacoin message handling failed", "err", err)
			return err
		}
	}
}
...
// BzcNodeInfo represents a short summary of the Bazacoin sub-protocol metadata known
// about the host peer.
type BzcNodeInfo struct {
	Network    uint64      `json:"network"`    // Bazacoin network ID (1=Frontier, 2=Morden, Ropsten=3)
...
func (self *ProtocolManager) NodeInfo() *BzcNodeInfo {
