...
// PeerInfo represents a short summary of the Bazacoin sub-protocol metadata known
// about a connected peer.
type PeerInfo struct {
	Version    int      `json:"version"`    // Bazacoin protocol version negotiated
...
// Handshake executes the bzc protocol handshake, negotiating version number,
...
func (p *peer) String() string {
	return fmt.Sprintf("Peer %s [%s]", p.id,
		fmt.Sprintf("bzc/%2d", p.version),
	)
}

// peerSet represents the collection of active peers currently participating in
// the Bazacoin sub-protocol.
type peerSet struct {
